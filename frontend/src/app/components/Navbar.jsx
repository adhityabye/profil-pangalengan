    ...
    <nav className="w-full fixed top-0 z-40 bg-main border-gray-200 h-14 md:h-16">
      <div className="max-w-screen-2xl h-full flex items-center justify-between mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="bg-secondary font-josefin px-8 py-2 mt-4 mr-8 text-base rounded-md text-center flex space-x-6 md:space-x-8 lg:space-x-10">
          {["Tentang", "Wilayah", "Rekomendasi Wisata", "Galeri"].map((text, index) => (
            <Link key={index} href={`/${text.toLowerCase().replace(" ", "-")}`} passHref>
              <h3
                className={`text-base text-white-100 font-semibold ${clicked === index ? "click-effect" : ""}`}
